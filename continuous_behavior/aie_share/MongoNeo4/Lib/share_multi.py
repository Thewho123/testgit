...
def BlockUpdate(dirname,Segname,blockdata,RecordingParam,sampling_rate):
    ...
    if Segname not in IndexName:
        ...
        blockdata.block_br.segments.append(Seg)

def GetEnsemble(ComparedSeg,SegnameList,blockdata):
    SegDict = {}
    for i in SegnameList:
        for Seg in blockdata:
            if i in Seg.name:
                SegDict[i]=Seg.spiketrains
    ...
    for j in SegDict.keys():
        if ComparedSeg==j:
            continue
        CompareDict[i].append(TwoSorterCompare(SegDict[ComparedSeg],SegDict[j]))
    if len(CompareDict[i])>4:
        SegNames = [i.name for i in blockdata.block_br.segments]
        MaxDict = {}
        ...
        for n,i in enumerate(blockdata.segments[ComparedSegIndex].spiketrains):
            if MaxDict['tdcSeg'][n]==0:
                continue
            if (MaxDict['tdcSeg'][n]<=2) and (i.description['snr']<5):
                continue
            spk_des = {'SimilarityLevel':MaxDict['tdcSeg'][n],
                       'SegID':n}
...
class BRshare():
    def __init__(self, raw_dirname='/home/cuilab/Desktop/Caeser_TDC/tdc_caeser202010153T001_lcy_peeler',
        ...
        event_time = blk_event_marker.get_event_timestamps()[0]/SampleRate*pq.sec
        ...
        if len(EnsembleIndex)!=0:
            ...
            EnsembleDataSpikeTrainID = [(ii.description['ElectrodeID'],ii.description['ElectrodeLabel']) for ii in EnsembleData.spiketrains]
