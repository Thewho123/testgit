...
def ReadKiloSort(kilo_dir):
    ...
    if len(glob.glob(os.path.join(kilo_dir,'cluster_info*')))!=0:
        ...
        KiloSortResult['group'] = list(info['group']) if 'KSLabel' not in info else list(info['KSLabel'])
...
def Kiloshare(KiloSortResult,nsFile,RecordingParam,sampling_rate=30000*pq.Hz):
    ...
    if '.ns6' in nsFile:
        RawData  = NsxFile(nsFile)
        RawDataChn = RawData.getdata(1)['data']
        RawData.close()
    
    if os.path.exists(os.path.join(nsFile,'waveform.mat')):
        wavemat = scio.loadmat(os.path.join(nsFile,'waveform.mat'))
        wavemat = wavemat[[i for i in wavemat if '__' not in i][0]]
        wavedict = {}
        for i in wavemat:
            wavedict[int(i[1].squeeze())] = i[3].squeeze()
    else:
        wavedict = {}
        wavedict[-1] = str(None)
    ...
    if 'id' in KiloSortResult.keys():
        for ind,i in enumerate(KiloSortResult['id']):
            ...
            if 'good' not in KiloSortResult['group'][ind]:
                continue
            try:
                fp = np.memmap(os.path.join(os.path.dirname(nsFile),'Phy','temp_wh.dat'), dtype='int16', mode='r', shape=(RawDataChn.shape[1],len(RawData.extended_headers)))
    
                RawDataArray = fp[:,KiloSortResult['ch'][ind]]
...
NeuralDataSegmentsIndex = {'tdc_spike_train' : 0,
                           'Kilo_spike_train': 1,
                           'Spykingcircus_spike_train' : 2,
                           'Herdingspike2_spike_train' : 3,
                           'IronClust_spike_train' : 4,
                           'HDSort_spike_train' : 5,
                           'Ensemble_spike_train':6,
                           'FinalEnsemble_spike_train':7,
                           'EMG':8,
                           'Motion':9,
                           'LFP':10,
                           'RecordingSystemEvent':11,
                           'BehaviorTrial':12
                           }

def FileConsolidation(dirname,Behaviorlist):
    FileList = os.listdir(dirname)
    NeuralDataPath = os.path.join(dirname,'NeuralData')
    BehaviorPath = os.path.join(dirname,'BehaviorData')
    if os.path.exists(BehaviorPath):
        return
    os.mkdir(NeuralDataPath)
    os.mkdir(BehaviorPath)
    for i in FileList:
        BehaviorFile = False
        for j in Behaviorlist:
            if j in i:
                BehaviorFile = True
        if BehaviorFile:
            shutil.move(os.path.join(dirname,i),BehaviorPath)
        else:
            shutil.move(os.path.join(dirname,i),NeuralDataPath)
...
class BaseShare():
    def __init__(self,
                 ...
                 waveclus_dir_output=True,
                 mlpath=None):
        
        #%% create block file
        description={'RawSampling_rate':sampling_rate}
        self.block_br = Block(name = BlockName,
        ...
        BehaviorName = os.path.join(raw_dirname,'BehaviorData') if os.path.exists(os.path.join(raw_dirname,'BehaviorData')) else raw_dirname
        self.mldata = BehaviorMetaDataInterface(BehaviorName,BlockName,mlpath).block
        
        dirnameList = [os.path.join(raw_dirname,'info.json'),
                       [os.path.join(raw_dirname,'Phy'),os.path.join(raw_dirname,'NeuralData','cluster_info.tsv')],
        ...
        for i,j in zip(dirnameList,SegnameList):
            ...
            if isinstance(i, list):
                tempFile=''
                for subi in i:
                    if os.path.exists(subi):
                        tempFile = os.path.join(raw_dirname,'NeuralData') if 'cluster_info.tsv' in subi else subi
                        flag = False
                i = tempFile
            else:
                if os.path.exists(i):
            ...
            print(i)
            print(j)
            BlockUpdate(i,j,self.block_br,RawRecordingParam,sampling_rate)
        
        #return
        #%% Ensemble
        SpikeSeg = [i for i in self.block_br.segments if 'spike_train' in i.name]
        if len(SpikeSeg)>4: GetEnsemble(ComparedSeg,SegnameList,self.block_br)
        ...
        #%%waveclus
        if len(EnsembleIndex)!=0 and waveclus_dir_output:
            ...
            if not os.path.exists(waveclus_dir): 
                ...
                #for i in tqdm(SelectedGroup): SaveMat(i,dataio_dir,waveclus_dir)
            ...
            SelectedGroup = np.unique([i.description['group'] for i in self.block_br.segments[SpikeTrainIndex[0]].spiketrains])
        #%%
        LFPSeg = Segment(name = 'LFP')
        #self.debuger = SelectedGroup
        if isinstance(LFPData,np.ndarray):
            description['LFPSampling_rate'] = LFPSampling_rate
            ChnDescription = [LFPRecordingParam[int(i)] for i in SelectedGroup]
            mdaList = LFPData[SelectedGroup].T     
            LFPsig = AnalogSignal(sampling_rate = LFPSampling_rate, 
                                  signal = mdaList,
            ...
            self.block_br.description = description
        ...
        if len(Motion_path_list)>0:
            ...
            if len(bhv_name)!=0:
                ...
                Motion_path_list = [i for i in Motion_path_list if i.split('/')[-1].split('.')[0]!=bhv_name[0]\
                                    and 'hdsort_output_results.mat' not in i and 'NeuralDataFile' not in i]
            else:
    ...
    def save_block(self):
        blockdata = {"block_br":self.block_br}
...
class BRshare(BaseShare):
    def __init__(self, raw_dirname='/home/cuilab/Desktop/Caeser_TDC/tdc_caeser202010153T001_lcy_peeler',
        ...
        try:
            ...
            LFPSampling_rate = LFPData['samp_per_s']*pq.Hz
            LFPData = LFPData['data']*pq.uV
            ns3_file.close()
        except:
